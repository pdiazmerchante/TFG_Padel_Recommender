...
def radar_player(player):
  data = df[df["jugador"] == player][pct_cols].mean()
  ...
  plt.title(f"Radar de {player}")
  plt.show()
  # Ejemplo para el primer jugador

  radar_player(df["jugador"].iloc[0])

# Ejemplo para el primer jugador
primer_jugador = df["jugador"].iloc[0] 
radar_player(primer_jugador)
```
...
sns.scatterplot(data=df, x="winner_%", y="error no forzado_%", hue="cluster")
plt.title("Clusterización de jugadores según rendimiento")
plt.show()
